...
export default function BlogFacebookAdsPage() {
  ...
  return (
    <main style={{ backgroundColor: "#f7f5f0", color: "#2d2d2d" }}>
          ...
          <Container>
            <div className="mx-auto max-w-3xl text-center">
              ...
              <div className="mb-4 flex items-center justify-center gap-2">
                <Link href="/blog/" className="text-sm text-[#6B7280] transition-colors hover:text-accent">
                  Blog
                </Link>
      ...
      {/* Stats rapides */}
      <section className="py-10" style={{ borderTop: "1px solid rgba(45,45,45,0.08)", borderBottom: "1px solid rgba(45,45,45,0.08)", backgroundColor: "#ede9e1" }}>
        <Container>
          <div className="grid grid-cols-2 gap-6 md:grid-cols-4">
            {[
              ...
              <div key={stat.label} className="text-center">
                ...
                <div className="font-heading text-2xl font-bold text-[#2d2d2d]">{stat.value}</div>
                <div className="mt-0.5 text-xs text-[#6B7280]">{stat.label}</div>
              </div>
            ))}
          </div>
        </Container>
        ...
        <Container>
          <div className="flex flex-col gap-12 lg:flex-row">
            ...
            <div className="flex-1">
              ...
              <p className="mb-10 font-heading text-3xl font-bold tracking-tight text-[#2d2d2d] sm:text-4xl">
                Maîtrisez Meta Ads
              </p>
              ...
              <div className="flex flex-col gap-6">
                {FACEBOOK_ADS_ARTICLES.map((article) => (
                  <article
                    key={article.slug}
                    className="group flex flex-col gap-4 rounded-xl border p-6 transition-all duration-300 hover:border-accent/20 hover:shadow-[0_0_30px_rgba(73,143,109,0.08)] sm:flex-row"
                  ...
                  >
                    <div className="flex-1">
                      <div className="mb-3 flex flex-wrap items-center gap-2">
                        ...
                        <span className="text-xs text-[#6B7280]">{article.readTime} de lecture</span>
                      </div>
                      <h3 className="font-heading text-lg font-semibold leading-snug text-[#2d2d2d] transition-colors group-hover:text-accent">
                        {article.title}
                      </h3>
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        {article.tags.map((tag) => (
                          <span
                            key={tag}
                            className="rounded border px-2 py-0.5 text-xs text-[#6B7280]"
                            style={{ borderColor: "rgba(45,45,45,0.10)", backgroundColor: "rgba(237,233,225,0.72)" }}
                          >
                            #{tag}
                          </span>
                        ))}
            ...
            <aside className="w-full lg:w-72 xl:w-80">
              {/* Sujets couverts */}
              <div className="rounded-xl border p-6" style={{ borderColor: "rgba(45,45,45,0.08)", backgroundColor: "rgba(255,255,255,0.72)" }}>
                <h3 className="mb-4 font-heading text-sm font-semibold uppercase tracking-wider text-accent">
                  Sujets couverts
                </h3>
                <ul className="space-y-2">
                  {TOPICS.map((topic) => (
                    <li key={topic} className="flex items-start gap-2 text-sm text-[#6B7280]">
              ...
              >
                ...
                <Link
                  ...
                  className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-lg px-4 py-3 text-sm font-semibold text-[#f7f5f0] transition-all duration-200 hover:brightness-110"
                ...
                <Link
                  ...
                  className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-lg border px-4 py-3 text-sm font-medium text-[#2d2d2d] transition-colors hover:border-accent/40 hover:text-accent"
                  style={{ borderColor: "rgba(45,45,45,0.10)", backgroundColor: "rgba(255,255,255,0.6)" }}
                >
              ...
              {/* Autres catégories */}
              <div className="mt-6 rounded-xl border p-6" style={{ borderColor: "rgba(45,45,45,0.08)", backgroundColor: "rgba(255,255,255,0.72)" }}>
                <h3 className="mb-4 font-heading text-sm font-semibold uppercase tracking-wider text-accent">
                  Autres catégories
                </h3>
                <div className="flex flex-col gap-2">
                  {[
                    ...
                    <Link
                      key={cat.slug}
                      href={`/blog/${cat.slug}/`}
                      className="group flex items-center justify-between rounded-lg border px-4 py-2.5 transition-all duration-200 hover:border-accent/20"
                      style={{ borderColor: "rgba(45,45,45,0.08)", backgroundColor: "rgba(237,233,225,0.72)" }}
                    >
                  ...
                  <Link
                    href="/blog/"
                    className="mt-1 text-center text-xs text-[#6B7280] transition-colors hover:text-accent"
                  >
      ...
      <section className="py-24">
        <Container>
          <div className="mx-auto max-w-3xl rounded-2xl border p-8 text-center sm:p-12" style={{ borderColor: "rgba(45,45,45,0.08)", backgroundColor: "rgba(255,255,255,0.72)" }}>
            ...
            <div className="mt-8 flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
              ...
              <Link
                href="/tarifs/facebook-ads/"
                className="inline-flex items-center gap-2 rounded-lg border px-8 py-4 text-lg font-medium text-[#2d2d2d] backdrop-blur transition-colors hover:border-accent/40 hover:text-accent"
                style={{ borderColor: "rgba(45,45,45,0.10)", backgroundColor: "rgba(255,255,255,0.6)" }}
              >
                Voir les tarifs
              </Link>
